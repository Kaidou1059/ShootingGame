...
public class ShootingView extends SurfaceView
    ...
    public ShootingView(Activity activity) {
        ...
        g.setOrigin((dh - H) / 2, 0);
        segmentFont = Typeface.createFromAsset(getContext().getAssets(), "Segment.ttf");
    ...
    public void drawScore(){
        ...
        g.drawText(num2str(score, 6),
                200+g.getOriginX(), 15-(int)g.getFontMetrics().ascent);
    }
    ...
    public void run() {
        while(thread != null) {
            ...
            g.drawText("SCORE " , 35+g.getOriginX(), 10-(int)g.getFontMetrics().ascent);

            finishTime = System.currentTimeMillis();
            long FPSnumber = finishTime - startTime;
            String FPS=String.valueOf(FPSnumber);
            g.drawText(FPS, 400+g.getOriginX(), 10-(int)g.getFontMetrics().ascent);
            g.unlock();
